...
var path = require('path'),
    ...
    Reporter = require('../../lib/report/flow'),
...
module.exports = {
    ...
    "should test files written": function (test) {
        ...
        test.ok(existsSync(reportDir));
        // Assets check
        test.ok(existsSync(fileFor('Inconsolata-Regular.ttf')));
        test.ok(existsSync(fileFor('SourceSansPro-Regular.ttf')));
        test.ok(existsSync(fileFor('jquery.scrollable.min.js')));
        test.ok(existsSync(fileFor('prettify-desert.css')));
        // Page generation check
    ...
    "should test files written with defaults": function (test) {
        var file = path.resolve(OUTPUT_DIR, 'coverage.json'),
            reportDir = path.resolve(process.cwd(), 'flow-report'),
    ...
    "test contents": function (test) {
        console.error('Figure out a way to run meaningful tests for flow report contents');
        test.ok(1);
        test.done();
    },
