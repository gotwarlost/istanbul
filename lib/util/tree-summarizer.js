...
TreeSummary.prototype = {
    ...
    convertToTree: function (summaryMap, arrayPrefix) {
        var nodes = [],
            ...
            prefix = arrayPrefix[0];

        // Removes the prefix and any separators from the name
        function fixPath(name) {
            if (prefix && name.indexOf(prefix) === 0) {
                name = name.substring(prefix.length);
            }
            while (name.charAt(0) === SEP) {
        ...
        // Is this needed?  What I'm finding is that this causes multiple root index files that overwrite each other in the report writing step
