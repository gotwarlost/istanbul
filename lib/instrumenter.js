...
(function(isNode) {
   ...
   function Instrumenter(options) {
      ...
      if (this.opts.map)
         map = this.opts.map; // override

      this.walker = new Walker(map, this.extractCurrentHint, this, this.opts.walkDebug);
   ...
   Instrumenter.prototype = {
      ...
      instrumentASTSync: function(program, filename, originalCode) {
         ...
         return this.getPreamble(originalCode || '', usingStrict, this.opts.preambleArray) + '\n' + ESPGEN.generate(program, codegenOptions) + '\n';
      },
      /**
      ...
      getPreamble: function(sourceCode, emitUseStrict, preambleArray) {
         ...
         if (preambleArray) {// we have an override for it
            array = array.concat(preambleArray);
         }
         else {
            array = array.concat([
