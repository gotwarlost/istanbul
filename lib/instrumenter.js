...
(function (isNode) {
    ...
    var SYNTAX,
        ...
        TRAILER_WRAP = '\n}());',
        COMMENT_RE = /^\s*istanbul\s+ignore\s+(if|else|last-else|next)(?=\W|$)/,
        astgen,
        preconditions,
        cond,
    ...
    Instrumenter.prototype = {
        ...
        ifBranchInjector: function (node, walker) {
            var alreadyIgnoring = !!this.currentState.ignoring,
                ...
                ignoreLastElse = !alreadyIgnoring && hint && hint.type === 'last-else',
            ...
            if (ignoreLastElse) { this.currentState.ignoreLastElse = { end: node.loc.end }; }

            if (this.currentState.ignoreLastElse && this.currentState.ignoreLastElse.end.line === node.loc.end.line) {
              child = node.alternate;
              if (child.body[0].type !== 'IfStatement') {
                bName = this.branchName('if', walker.startLineForNode(node), [
                    { start: makeLoc(), end: makeLoc() },
                    { start: makeLoc(), end: makeLoc(), skip: true }
                ]);
