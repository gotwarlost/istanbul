...
Command.mix(ReportCommand, {
    ...
    usage: function () {
        console.error('\nUsage: ' + this.toolName() + ' ' + this.type() + ' <options> [ <format> ... ]\n\nOptions are:\n\n' +
            [
                ...
                formatOption('--compilers <ext>:<module>,...', 'Use the given module(s) to compile files'),
    ...
    run: function (args, callback) {
        ...
        reports.run(formats, config, { include: opts.include, root: opts.root, compilers: compilers }, callback);
    }
});
