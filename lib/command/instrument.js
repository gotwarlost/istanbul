...
function processFiles(instrumenter, inputDir, outputDir, relativeNames, prefix) {
    ...
    var processor = function (name, callback) {
            ...
            if (isJavaScriptFile) {
                fs.readFile(inputFile, 'utf8', function (err, data) {
                    ...
                    instrumenter.instrument(data, (prefix ? inputFile.replace(prefix, ".") : inputFile), function (iErr, instrumented) {
    ...
    usage: function () {
        console.error('\nUsage: ' + this.toolName() + ' ' + this.type() + ' <options> <file-or-directory>\n\nOptions are:\n\n' +
            [
                ...
                formatOption('--prefix <path>', 'the prefix of js source file')
            ].join('\n\n') + '\n');
        console.error('\n');
    },
    ...
    run: function (args, callback) {
        ...
        if (stats.isDirectory()) {
            ...
            filesFor({
                ...
                processFiles(instrumenter, file, output, files, opts['prefix']);
            });
        } else {
            if (output) {
