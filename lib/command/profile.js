...
Command.mix(ProfileCommand, {
   ...
   coverFunctionProfiler: function(node, walker) {
      // we need function name
      var fnName = node.id && node.id.name;

      if (!fnName) {
         // try to grab function name differently
         ...
         if (parent.node.type === "AssignmentExpression") {
            if (parent.node.left.type === "MemberExpression") {
               ...
               if (parent.node.left.object.name)
                  fnName += parent.node.left.object.name + ".";
               fnName += parent.node.left.property.name || parent.node.left.property.value;
            }
         } else {
            if (parent.node.type === "Property") {
               fnName = parent.node.key.name;
            }
         }
      }

      if (fnName) {
         // if we have function name, lets modify the function body

         // inject profile.start(functionName, __filename);
         ...
         // wrap the full body with try {...} finally {profile.end(functionName, __filename);}
         node.body.body = [
            {
               "type": "TryStatement",
               "block": {
