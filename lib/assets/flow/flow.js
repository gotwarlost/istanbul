...
$(document).ready(function () {
    ...
    function loadData() {
        var data = JSON.parse(sessionStorage.getItem('istanbul.flow') || '{}'),
    ...
    function loadSummaryData(pages, path) {
        ...
        rows.each(function () {
            var cells = $(this).children(),
                file = cells.filter('.file').children('a').get(0).textContent,
    ...
    function saveSummaryData(data, detailPage, loading) {
        ...
        if (!loading) {
            oldSummary = Object.assign({}, data.summary);
        }
