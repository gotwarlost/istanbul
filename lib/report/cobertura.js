...
function branchCoverageByLine(fileCoverage) {
    ...
    Object.keys(branchMap).forEach(function (k) {
        ...
        ret[line] = ret[line] || [];
        branches[k].forEach(function(data, index) {
            if (!branchMap[k].locations[index].skip) {
                ret[line].push(data); // only push branches which were not ignored
            }
        });
    });
    Object.keys(ret).forEach(function (k) {
        ...
        // Fallback for ignored branches
        if (covered.length === 0 && dataArray.length === 0) {
            coverage = 100;
        }
...
function addClassStats(node, fileCoverage, writer, projectRoot) {
    ...
    Object.keys(fnMap).forEach(function (k) {
        ...
        writer.println(
            ...
            attr('hits', fnMap[k].skip ? 1 : fileCoverage.f[k]) +
            '/>' +
            '</lines>'
        );
