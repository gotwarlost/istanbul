...
function HtmlReport(opts) {
    ...
    this.opts.remap = this.opts.remap || false;
}
...
Report.mix(HtmlReport, {
    ...
    getSourceCode: function (filePath) {
        // You may ask why not to use fs.readFileSync
        // Babel replaces it exclusively for istanbul
        // so we can never get ES6 source with fs.rFS
        // And that is why we need to use fs.readSync
        var buffers = [];
        var pos = 0;
        var buffer;
        var fd;
        ...
        while (!done) {
            ...
            read = fs.readSync(fd, buffer, 0, 8192);

            if (read) {
              buffers.push(buffer.slice(0, read));
            }
    ...
    writeDetailPage: function (writer, node, fileCoverage) {
        var opts = this.opts,
            ...
            sourceText = fileCoverage.code && Array.isArray(fileCoverage.code) ?
                fileCoverage.code.join('\n') + '\n' : opts.remap ? this.getSourceCode(fileCoverage.path) : sourceStore.get(fileCoverage.path),
