...
var headerTemplate = templateFor('head-flow'),
    ...
    detailTemplate = handlebars.compile(
        ...
        '<td class="code"><pre class="prettyprint lang-js">{{#code structuredText}}{{/code}}</pre></td>\n' +
        '</tr>\n'
    ),
    ...
    summaryTableHeaderTemplate = handlebars.compile(
        '<table class="summary">\n' +
        '<thead>\n' +
        '<tr>\n' +
        '<th data-col="file" data-fmt="html" data-html="true" class="file">{{kind}}</th>\n' +
...
function annotateBranches(fileCoverage, metrics, structuredText) {
    ...
    Object.keys(branchStats).forEach(function (branchName) {
        ...
        for (var i = 0; i < branchArray.length; i += 1) {
...
Report.mix(FlowReport, {
    ...
    getPathHtml: function (node, linkMapper) {
        ...
        for (var i = 0; i < nodePath.length; i += 1) {
        ...
        return linkPath.length > 0 ?
            linkPath.join('') + cleanPath(node.displayShortName()) :
            '/';
    },

    popuplateTemplateData: function (node, data) {
        var opts = this.opts,
            linkMapper = opts.linkMapper;

        data.timestamp = timestamp;
        data.path = '/' + node.name;
        data.kind = data.path === '/' ? 'Directory' : 'File';
        data.bodyClass = node.kind === 'dir' ? 'summary-page' : 'detail-page';
        data.metrics = node.metrics;
        populateMetrics(data.metrics);
        data.pathHtml = this.getPathHtml(node, linkMapper);
        data.base = {
            css: linkMapper.asset(node, 'base.css')
        };
        data.jquery = { js: linkMapper.asset(node, 'jquery-2.2.3.min.js') };
        data.documentsize = {
    ...
    writeIndexPage: function (writer, node) {
        ...
        this.popuplateTemplateData(node, templateData);
        writer.write(headerTemplate(templateData));
        writer.write(summaryTableHeaderTemplate(templateData));
    ...
    writeReport: function (collector, sync) {
        ...
        ['flow', path.join('flow', 'vendor'), 'vendor'].forEach(function (subdir) {
            copyAssets(subdir);
        });
